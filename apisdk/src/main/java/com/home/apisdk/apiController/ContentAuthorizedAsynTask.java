 ...
 * @author MUVI
 */
public class ContentAuthorizedAsynTask extends AsyncTask<ValidateUserInput, Void, Void> {
    ...
    private ContentAuthorizedListener listener;
    private Context context;

    /**
     * Interface used to allow the caller of a ContentAuthorizedAsynTask to run some code when get
     * responses.
     */

    public interface ContentAuthorizedListener {
        ...
        void onContentAuthorizedPreExecuteStarted();
        ...
        void onContentAuthorizedPostExecuteCompleted(ValidateUserOutput validateUserOutput, int status, String message);
    }
    ...
    public ContentAuthorizedAsynTask(ValidateUserInput validateUserInput, ContentAuthorizedListener listener, Context context) {
    ...
    protected void onPreExecute() {
        super.onPreExecute();
        listener.onContentAuthorizedPreExecuteStarted();

        status = 0;
        if (!PACKAGE_NAME.equals(SDKInitializer.getUser_Package_Name_At_Api(context))) {
            this.cancel(true);
            message = "Packge Name Not Matched";
            listener.onContentAuthorizedPostExecuteCompleted(validateUserOutput, status, message);
            return;
        }
